...
public class EquateEncodedStrings {

    public static void main(String[] args) {
        EquateEncodedStrings strEquals  = new EquateEncodedStrings();
        strEquals.testEquality();
        int[][] array = new int[5][2];
        //Arrays.sort(array, Comparator.comparingInt(a -> a[0]).reverseOrder());
    }

    private void testEquality() {
        String str1 = "internationalization";
        String str2 = "i18n";
        System.out.printf("%s and %s are %b\n", str1, str2, possiblyEquals(str1, str2));
    }
    ...
    private boolean recurse(int i1, int wildcards1, int i2, int wildCards2) {
        ...
        final List<Integer> memoState = Arrays.asList(i1, wildcards1, i2, wildCards2); // memo to optimize failed state
        if (failMemo.contains(memoState)) return false; // check memo

        // generate all possible wildcards from numerical values
        Set<int[]> skips1 = !isEnd(i1, string1) && !isLetter(i1, string1) ? extractSkips(string1, i1) : new HashSet<>(Arrays.asList(singleSkip));
        Set<int[]> skips2 = !isEnd(i2, string2) && !isLetter(i2, string2) ? extractSkips(string2, i2) : new HashSet<>(Arrays.asList(singleSkip));
